...
import { getConfig, type BaseFunctionConfig } from "@khulnasoft/static-config";
...
export function khulnasoftPreset(): Preset {
  let project = new Project();
  let khulnasoftEntryServerPath: string | undefined;
  ...
  // `entry.server` file needs use the `@khulnasoft/remix` package.
  //
  //  - If there is no `entry.server` file, then we copy in the Khulnasoft entry server
  ...
  let createServerBundles =
    ...
    ({ branch }) => {
      ...
      if (config.runtime === "edge") {
        injectKhulnasoftEntryServer(remixUserConfig);
      }
  ...
  let buildEnd: VitePluginConfig["buildEnd"] = ({
    ...
    if (khulnasoftEntryServerPath) {
      rmSync(khulnasoftEntryServerPath);
    ...
    mkdirSync(".khulnasoft", { recursive: true });
    writeFileSync(".khulnasoft/remix-build-result.json", `${json}\n`);
  };
  ...
  return {
    ...
    remixConfig({ remixUserConfig }) {
      return {
         ...
         *     to the `.khulnasoft/remix-build-result.json` file, including
